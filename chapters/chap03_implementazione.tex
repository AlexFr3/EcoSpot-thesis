...
\subsection{Sistema di Gamification}
...
\subsubsection{Gestione Utente e Rango}
...
\subsection{Integrazione Dati Sensori (IoT)}
...
if (response.statusCode == 200) {
  ...
  \end{minted}
  \caption{Parsing dei dati sensore in Dart}
...
\section{Interfaccia Utente e Componenti Core}
...
Per gestire la reattività dell'applicazione, è stato utilizzato il pattern Provider. La classe \texttt{AppProviders} centralizza la logica di business, permettendo ai widget di ascoltare i cambiamenti nei dati (come una nuova scoperta o l'aggiornamento dei sensori) e di ricostruirsi automaticamente solo quando necessario, ottimizzando le risorse del dispositivo.